...
void add_vector(V output, V v1, V v2) {
   ...
   for (i = 0; i <= v1.size() - (v1.size()%SIMD_SIZE); i += SIMD_SIZE) {
      __mx a = simd_loadu(&v1[i]);
      __mx b = simd_loadu(&v2[i]);
      __mx c = simd_add(a, b);
      simd_storeu(&output[i], c);
   }
   for (; i < v1.size(); i++) {
      output[i] = v1[i] + v2[i];
   }
      ...
      for (int i = 0; i < fftPts; ++i) { // m = fftPts
         for (int j = 0; j < cols; ++j) { // n = cols
            ...
            for (int k = 0; k < rows-(rows%SIMD_SIZE); k += SIMD_SIZE) { // k = rows
                  a = simd_loadu(&input[i * rows + k]);
                  b = simd_loadu(&weights[j * rows + k]);
                  sum = simd_add(sum, simd_mul(a, b));
            }
            float tmp[SIMD_SIZE];
            simd_storeu(tmp, sum);
            for (int l = 0; l < SIMD_SIZE; l++) {
               output[i*cols+j] += tmp[l];
            }
