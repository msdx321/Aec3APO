...
void pRelu(V x, float a) {
   __mx a_vec = simd_set1(a);
   __mx zero = simd_setzero();

   for (size_t i = 0; i < x.size()-(x.size()%SIMD_SIZE); i += SIMD_SIZE) {
      __mx x_vec = simd_loadu(&x[i]);
      __mx mask = simd_cmpge(x_vec, zero);
      __mx result = simd_or(
         simd_and(mask, x_vec),
         simd_andnot(mask, simd_mul(a_vec, x_vec))
      );
      simd_storeu(&x[i], result);
   }

   // Handle remaining elements
   for (size_t i = x.size() - (x.size() % SIMD_SIZE); i < x.size(); i++) {
      x[i] = x[i] >= 0 ? x[i] : a * x[i];
   }
}
