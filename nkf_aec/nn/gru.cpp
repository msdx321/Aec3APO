...
void GRUCell::forward(V input, V h_prev_t) {
   // h, d are ints
   // N = 513, L = 1, Hin = 18
   // since we're dealing only with L = 1 the output IS h_n
   for (int i = 0; i < numBins; i++) {
      dot_product(r1, Wrx, h, d, input, brx, i*d);
      dot_product(r2, Wrh, h, d, input, brh, i*d);
      add_vector(r, r1, r2);
      sigmoid(r);
      // for (int i = 0; i < h; i++) {
      ...
      //    }
      // }
      dot_product(z1, Wzx, h, d, input, bzx, i*d);
      ...
      //    }
      // }
      dot_product(n1, Wnx, h, d, input, bnx, i*d);
